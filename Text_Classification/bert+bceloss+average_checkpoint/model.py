...
class Model(nn.Module):
    def __init__(self):
        ...
        self.wobert = BertModel.from_pretrained(
            "./wobert_pretrain/pytorch_model.bin", config=self.config
        )
        self.output = nn.Linear(self.config.hidden_size, 1)

    def forward(
        self, input_ids=None, attention_mask=None, segment_ids=None, labels=None
    ):
        sequence_output, cls_output = self.wobert(
            input_ids=input_ids,
